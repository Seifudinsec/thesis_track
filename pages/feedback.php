...
$stmt = $pdo->prepare("SELECT s.*, u.full_name as student_name FROM submissions s JOIN users u ON s.student_id = u.id WHERE s.id = ?");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit_feedback'])) {
    ...
    try {
        ...
        if ($existing_feedback) {
            $stmt = $pdo->prepare("UPDATE feedback SET comments = ?, grade = ?, supervisor_id = ? WHERE submission_id = ?");
            $stmt->execute([$comments, $grade, $supervisor_id, $submission_id]);
        } else {
            $stmt = $pdo->prepare("INSERT INTO feedback (submission_id, supervisor_id, comments, grade) VALUES (?, ?, ?, ?)");
...
<div style="max-width: 800px; margin: 0 auto;">
    ...
    <div class="card" style="margin-top: 2rem;">
        ...
        <a href="../<?php echo $submission['file_path']; ?>" target="_blank" class="btn btn-action" style="margin-top: 1rem; display: inline-block;">Download & Review PDF</a>
    </div>
